...
int main(){
	...
	int diff;
	int lca;
	for (int i = 0; i < 18; i++)bs[1 << i] = i;
	for (int i = 1; i < for_bs; i++){
		if (bs[i] == 0)bs[i] = bs[i - 1];
	}
	vector<int> ans;
	scanf("%d%d%d",&n,&m,&q);
	for (i = 0; i < n - 1; i++){
	...
	while (q--){
		...
		j = v;
		ans = people[lca];
		if (depth[u] - depth[lca] != 0){
			ans = merge(ans, candidate[u][bs[depth[u] - depth[lca]]]);
			diff = depth[u] - depth[lca] - (1<<bs[ depth[u] - depth[lca] ]);
			if (diff != 0){
				i = climb(u, diff);
				ans = merge(ans, candidate[i][bs[depth[i] - depth[lca]]]);
			}
		}
		if (depth[v] - depth[lca] != 0){
			ans = merge(ans, candidate[v][bs[depth[v] - depth[lca]]]);
			diff = depth[v] - depth[lca] - (1<<bs[depth[v] - depth[lca]]);
			if (diff != 0){
				j = climb(v, diff);
				ans = merge(ans, candidate[j][bs[depth[j] - depth[lca]]]);
			}
		}
		/*k = 0;
